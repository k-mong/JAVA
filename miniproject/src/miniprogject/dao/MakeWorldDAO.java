...
public class MakeWorldDAO {
	
	private MakeWorldDAO() {}
	
	private static MakeWorldDAO instance = new MakeWorldDAO();
	
	public static MakeWorldDAO getInstance() {
		return instance;
	}
	 ...
	 * 화면에서 데이터 넣기
	 */
	public void insertWorld(WorldcupVO worldcup) {
		Connection conn = null;
		PreparedStatement pstmt = null;	
		String sql = "INSERT INTO myWorld VALUES(?, ?)";

		try {
			conn = ConnectDb.getConnection();
			
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, worldcup.getName());
			pstmt.setString(2, worldcup.getPictureurl());
	...
	public List<WorldcupVO> selectRandom(){
		...
		List<WorldcupVO> makeList = new ArrayList<WorldcupVO>();	//List에 담는다
